...
void PlanetSettings::Random(QVector<bool> isRnd,PointChoicer* pc){
    ...
    if (isRnd[13]) is_plant=RAND(0,1);
    if (isRnd[14]) shine=RAND(0,5);
    if (isRnd[15]) point_of_shine_true=pc->GetRand();
    if (isRnd[16]) name_algorithm=RAND(1,3);
    if (isRnd[17]) is_cloud=RAND(0,1);
    if (isRnd[18]) cloud_size=RAND(1,20);
    if (isRnd[19]) cloud_quality=RAND(1,10);
    if (isRnd[20]) cloud_transparent=RAND(0,10);
    if (isRnd[21]) correction=RAND(0,1);
    if (isRnd[22]) cloud_color=QColor(RAND(0,255),RAND(0,255),RAND(0,255));
    if (isRnd[23]) is_atmo=RAND(0,1);
    if (isRnd[24]) atmo_transparent=RAND(0,10);
    if (isRnd[25]) atmo_size=RAND(1,10);
    if (isRnd[26]) atmo_color=QColor(RAND(0,255),RAND(0,255),RAND(0,255));
    if (isRnd[27]) is_ring=RAND(0,1);
    if (isRnd[28]) R_internal_ring=RAND(1,5);
    if (isRnd[29]) R_external_ring=RAND(1,5);
    if (isRnd[30]) ring_color=QColor(RAND(0,255),RAND(0,255),RAND(0,255));
    if (isRnd[31]) point_of_polar_true=pc->GetRand();
}
