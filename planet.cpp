...
void Planet::Civilization()
{
    ...
    boxBlurWrapX(waterNear, nearR);
    boxBlurClampY(waterNear, nearR);
    ...
    for (const Cand &c : cand)
    {
        ...
        for (int ox = -1; ox <= 1; ++ox)
        {
            for (int oy = -1; oy <= 1; ++oy)
            {
                ...
                if (ox == 0 && oy == 0)
                    img.setPixelColor(px, py, glow);
                else
                    img.setPixelColor(px, py, TransparentColor(img.pixelColor(px, py), glow, 0.55));
            }
        }
    }
}
